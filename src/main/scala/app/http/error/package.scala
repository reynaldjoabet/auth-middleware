...
/**
  ...
  *
  * One given instance per error type — each feature owns the error → status decision for its *own*
  * errors, while the wire format ([[ProblemDetails]]) and rendering ([[ErrorMapper.toResponse]])
  * stay central. Adding a new error type means adding a `given`, never editing a shared match.
  */
...
object ToProblemDetails {
  ...
  /**
    * Generic cross-cutting errors. Feature-specific error types provide their own instance (ideally

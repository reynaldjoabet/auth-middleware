...
/**
  * Stable, low-cardinality code for an authentication outcome, used as the audit decision. Bounded
  * by construction — never includes claim values.
  */
...
/**
  * An immutable audit record for one authentication decision. Carries only redaction-safe
  * identifiers (subject / client / token id), never token material or claim values.
  */
...
/**
  * Append-only sink for audit records. Back it with durable, tamper-evident storage (WORM bucket,
  * append-only table, SIEM) for compliance.
  */
trait Audit[F[_]] {
...
object Audit {
  ...
  /**
    * In-memory sink, for tests and local development. Returns the sink plus a reader that yields
...
/**
  * Adapts an [[Audit]] sink to the [[AuthEvents]] seam: every granted or denied decision is written
  * to the audit trail, then forwarded to `underlying`.
  */

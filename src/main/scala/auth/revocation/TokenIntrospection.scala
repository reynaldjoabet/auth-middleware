...
/**
  * Revocation checking via OAuth 2.0 Token Introspection (RFC 7662) — the Duende pattern for
  * revocation at a resource server: instead of every RS maintaining a shared denylist (Redis), ask
  * the authorization server, which already owns revocation state. This removes the last piece of
  * shared infrastructure from the middleware.
  *
  * Trade-offs, mirroring `@RequireOAuth2(introspect = true)` on the Java side and Duende's
  * guidance:
  *   - a network hop per (uncached) check — gate it on the sensitive route groups rather than the
  *     whole API if latency matters; construct one [[AccessTokenValidator]] with introspection for
  *     those routes and one without for the rest
  *   - failures are reported as [[Result.Unavailable]] and the validator fails closed (503,
  *     `Retry-After`) — a token we cannot prove active is not accepted
  *   - with `cacheTtl > 0` a positive result is reused, so revocation takes effect within
  *     `cacheTtl` at worst; set it to zero for hard real-time revocation at full network cost
  */
...
object TokenIntrospection {
  ...
  /**
    * @param endpoint
    *   the AS's introspection endpoint; https only — credentials and tokens transit this connection
    * @param clientId
    *   this resource server's client id at the AS (RFC 7662 requires the caller to authenticate,
    *   otherwise introspection is a token oracle)
    * @param clientSecret
    *   secret for HTTP Basic authentication at the endpoint
    * @param cacheTtl
    *   how long a definitive answer (active / inactive) is reused for the same token. Bounds
    *   worst-case revocation latency; zero disables caching.
    * @param requestTimeout
    *   hard deadline for one introspection round trip. A stalled AS must not pin request threads:
    *   on expiry the check is [[Result.Unavailable]] and the caller fails closed.
  ...
  /**
    * Cache key: base64url(SHA-256(token)) — the raw token never sits in memory beyond the request
    * that carried it.
    */

...
/**
  * A distributed [[auth.dpop.DpopNonceStore]] backed by Redis/Valkey via Sage, for deployments
  * whose threat model demands strictly single-use DPoP nonces *across* nodes — a client's retry may
  * land anywhere and a nonce still verifies at most once cluster-wide. Wrap it with
  * [[auth.dpop.DpopNonceValidator.fromStore]].
  *
  * If freshness-window semantics are acceptable, prefer [[auth.dpop.DpopNonceValidator.stateless]]
  * (Duende's design) and skip the shared store entirely.
  *

...
enum AuthError derives CanEqual {
  ...
  /**
    * The request shape itself is unacceptable — for example the access token was passed in the
    * query string (forbidden by OAuth 2.1, and a leak vector via logs, referrers and browser
    * history) or multiple credentials were supplied. RFC 6750 `invalid_request`, answered with 400.
    */
  case InvalidRequest(reason: String)

  /**
    * The token failed structural, signature, type, claims or sender-constraint validation, or has
    * been revoked. RFC 6750 `invalid_token`.
    */
  case InvalidToken(reason: String)

  /**
    * The DPoP proof JWT accompanying the request was missing, malformed, stale, replayed or
    * otherwise invalid. RFC 9449 `invalid_dpop_proof`.
    */
  case InvalidDpopProof(reason: String)

  /**
    * The resource server requires a DPoP proof carrying a server-provided nonce (RFC 9449 §8-9),
    * and the proof either omitted it or presented one that is unknown, expired or already used.
    * Answered with `401`, `error="use_dpop_nonce"` and a fresh `DPoP-Nonce` response header the
    * client must echo in the `nonce` claim of its next proof.
    *
    * This is the fix mandated by the FAPI 2.0 formal analysis for the DPoP Proof Replay attack:
    * without it, a network attacker who reads a leaked resource request (or blocks the honest one)
    * can replay the proof, since per-node single-use detection never sees the original request.
    * Unlike the other `InvalidDpopProof*` reasons this is not really a client error — it is a
    * challenge — so it carries the nonce to hand back rather than a fixed `reason` string.
    */
  case UseDpopNonce(nonce: DpopNonce)

  /**
    * The token is valid but does not carry the scopes the route requires. RFC 6750
    * `insufficient_scope`.
    */
  case InsufficientScope(required: Set[String])

  /**
    * The token is valid but the user's authentication does not meet the route's step-up
    * requirements (`acr` value and/or `auth_time` freshness). RFC 9470
    * `insufficient_user_authentication`.
    *
    * `acrValues` is ordered by preference (RFC 9470 §3: the `acr_values` challenge parameter lists
    * values "in order of preference"), so it is a `Seq`, not a `Set`.
    */

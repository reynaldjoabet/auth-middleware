...
/**
  ...
  *     impossible because only signed JWTs are parsed)
  ...
  *   permitted JOSE `typ` header values. RFC 9068 access tokens use `at+jwt`; plain `JWT` (or an
  *   absent `typ`) is accepted by default for compatibility. Restrict to `at+jwt` only if your
  *   issuer supports it, to rule out ID-token replay at the API.
  * @param clockSkew
  *   tolerated clock difference when checking `exp` / `nbf` / `iat`
  * @param requiredClaims
  *   claims that must be present for the token to be accepted. The default is the RFC 9068 §2.2
  *   required set for JWT access tokens: `sub`, `exp`, `iat`, `client_id` and `jti`. (`iss` and
  *   `aud` are also mandatory but are enforced separately — by the exact-match issuer check and the
  *   audience check.) Relax this set if you must accept tokens from a non-9068-compliant issuer.
  *   Keep `jti` here when using a revocation denylist so tokens cannot dodge the check by omitting
  *   it.
  ...
  * @param jwksOutageTtl
  *   how long previously fetched (public) keys may continue to be used if the JWKS endpoint is
  *   down; after this, validation fails closed

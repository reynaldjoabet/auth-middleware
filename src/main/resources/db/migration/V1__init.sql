-- Flyway runs each migration inside its own transaction, so this script must
-- not open one of its own: an explicit COMMIT here would end Flyway's
-- transaction mid-migration and defeat the all-or-nothing rollback.
